...
ggplot(mtcars, aes(x = wt, y = mpg, color = cyl, shape = cyl)) + geom_point() + labs(x = "Weight of the car", y = "Miles per galon", title = "Relation between weight and fuel economy in cars of different number of cylinders")
...
graf <- ggplot(mtcars, aes(x = wt, y = mpg)) + geom_point(color = "blue")
graf
```
...
mtcars %>% ggplot(aes(x = wt, y = after_stat(ncount))) + geom_histogram(bins = 4)
...
ggplot(diamondsSample, aes(x*y*z, price, color = color)) + geom_point(alpha = 0.4) + labs(
  x="volumen u mm3",
  y="cijena u $",
  titl="kvaliteta boje"
)
...
```{r}
diamonds %>% ggplot(aes(x = price)) + geom_histogram(bins = 10) 
diamonds %>% ggplot(aes(x = price)) + geom_freqpoly(bins = 10) 
```
...
```{r}
diamonds %>% ggplot(aes(x = price)) + geom_histogram(bins = 10, aes(fill = clarity)) 

diamonds %>% ggplot(aes(x = price)) + geom_freqpoly(bins = 10, aes(color = clarity)) 
...
mpg %>% ggplot(aes(x = displ, y = cty)) + geom_point(shape = 8, aes(color = class)) + geom_smooth(linetype = "dashed", color = "grey", alpha = 0.5, se = F) + labs(x = "Engine displacement / liters", y = "City miles per gallon")
...
mpg %>% ggplot(aes(x = drv, y = hwy)) + geom_boxplot() + labs(x = "Drive type", y = "Highway miles per gallon")
...
```{r}
# create a stacked histogram of ratios by drive type per car manufacturer 

mpg %>% ggplot(aes(x = drv, fill = manufacturer)) + geom_bar(position = "stack", color = "white") + labs(x = "Drive type", y = "ratio")
...
ggplot(mpg, aes(x = as.factor(cyl), 
                fill = as.factor(year))) + 
  geom_bar() + scale_x_discrete(name = "broj cilindara") +
  scale_y_continuous(name = "ukupno", limits = c(0, 100)) +
  scale_fill_brewer(name = "godina", palette = "Dark2")
...
ggplot(mpg, aes(x = as.factor(trans), y = displ)) + 
  geom_boxplot() + theme(axis.text.x = element_text(angle = 90)) + theme_bw()
```
...
g1 <- ggplot(mpg %>% filter(cyl == 4), aes(hwy)) + geom_histogram(bins = 10) + labs(title = "4 cilindra")
g2 <- ggplot(mpg %>% filter(cyl == 5), aes(hwy)) + geom_histogram(bins = 10) + labs(title = "5 cilindara")
g3 <- ggplot(mpg %>% filter(cyl == 6), aes(hwy)) + geom_histogram(bins = 10) + labs(title = "6 cilindara")
g4 <- ggplot(mpg %>% filter(cyl == 8), aes(hwy)) + geom_histogram(bins = 10) + labs(title = "8 cilindara")
...
```{r}
prodaja %>% ggplot(aes(x = as.factor(mjesec), y = ukupno)) + geom_bar(stat = "identity") + labs(x = "Mjesec", y = "Ukupno")
```
