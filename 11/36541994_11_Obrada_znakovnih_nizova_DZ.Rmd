---
title: "11 Obrada znakovnih nizova - ZADAĆA"
...
```{r}
crimes$Text_General_Code %>% tolower %>% str_detect("burglary|robbery") %>% 
  sum %>% '/'(nrow(crimes))

str_count("banana", c("a","n"))
...
```{r}
crimes$Location_Block %>% str_extract_all("[:digit:]{4}") %>% unlist %>% unique
```
