...
---
title: "3 Vektori, matrice i liste RB"
author: "36541994 Karlo Vrančić"
date: "17. listopad 2023."
output:
...
Jer se TRUE konvertirao u character type, a "true" string nema numeričku vrijednost
...
a / b        # REPLICIRA DVAPUT b PA DIJELI PODUDARAJUĆE  

# pomnožite vektore `a` i `c` i ispišite rezultat
a * c        # WARNING JER DULJINE NISU VIŠEKRATNE - SJETI SE RECIKLIRANJA          
...
x[-5] #vraća cijelo polje bez elementa na tom indeksu
...
b # vrijednost 0 će poprimiti elementi s indeksom 2*1, 2*2, 2*3, 2*4, 2*5
...
y <- c(T, F, F, T, T, F, F, T, F, T)

# indeksirajte vektor `x` vektorom `y`, ispišite i objasnite rezultat
x[y] # ispusuju se indeksi na kojima je TRUE 
...
matrix(x, nrow = 3, ncol = 4) # popunjava ga po stupcima 
...
matrix(x, nrow = 3, ncol = 4, byrow = T) # popunjava ga po retcima

# --PO DEFAULTU SE ELEMENTI SLAZU PO STUPCIMA -> PROMIJENITI BYROW = T
...
dim(m) # vektor preko dima postaje matrica 
...
Vidimo da su retci i stupci "naslijedili" imena vektora od kojih su nastali, što je ponekad zgodno kao vizualni podsjetnik ili jednostavnije referenciranje. Ukoliko želimo promijeniti (ili ukloniti) imena redaka ili stupaca, umjesto funkcije `names` kod matrica koristimo funkcije `rownames` ili `colnames`.
...
m[,]                            # svi retci i svi stupci (može i samo `m` -> EKVIVALENTNO)
# da naprimjer napisem m[1,] -> to postaje vektor, ako zelimo da ostane matrica onda m[1, drop = F]
...
Polja su koristan tip objekta za višedimenzionalne izračune i specijalizirane svrhe, npr. kod programiranja s tenzorima. Bitno je shvatiti da se usprkos dodatnoj kompleksnosti koju donosi višedimenzionalnost veliki dio funkcionalnosti direktno preslikava iz funkcija za upravljanje nižedimenzionalnim objektima, možda uz manje preinake - npr. za imenovanje "slojeva" polja nećemo koristiti `names` niti `rownames`/`colnames` već funkciju `dimnames` uz eksplicitno navođenje indeksa dimenzije koju imenujemo.
...
LISTA SE INDEKSIRA SA DVIJE UGLATE 
***
...
# provjerite njegov tip -> SVI ELEMENTI LISTE SU MALE LISTE
typeof(svastara[1])
